# `scripts/pe/` — PyPSA-Earth CONUS prenetwork (WP1 pipeline test)
...
- Cost data are the generic technology-data set (`costs.country_specific_data: ""`), not the US-specific
  ATB-derived one pypsa-earth also offers; the study's own cost baseline is in `scripts/sep/tech`.
