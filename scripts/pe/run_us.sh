...
#   bash scripts/pe/run_us.sh [prestage] [US-smoke] [US]     (default: all three, in order)
...
run_stage() {   # $1 = run name: US-smoke | US
  ...
  for attempt in 1 2 3; do
    ...
    if [ "$rc" -eq 137 ] || grep -qE "SCOPE .*oom_kill [1-9]|MemoryError|Killed" <(tail -n 300 "$slog"); then
    ...
    rule=$(grep -oP "Error in rule \K\w+" "$slog" | tail -1)
