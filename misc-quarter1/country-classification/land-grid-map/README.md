...
This directory holds only the static page and its data. It was its own git repository
(LukasFrankenQ/country-classification) until September 2026 and was folded into the catalyst repo
with `git subtree add`, history included. The scripts that produce the data live one level up in
`misc-quarter1/country-classification` and write into `data/` here. Run them from there:
...
After regenerating, commit the changed files in the catalyst repo and push; Vercel redeploys from it
(project root directory `misc-quarter1/country-classification/land-grid-map`).

## Deploying

The page is static: deploy this directory as-is (for Vercel, `vercel deploy` from
`misc-quarter1/country-classification/land-grid-map`, no build step, output directory `.`). Only Leaflet and the basemap
